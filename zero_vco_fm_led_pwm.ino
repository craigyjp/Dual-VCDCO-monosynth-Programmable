...
void setup()
{
  pinMode(OCT_SEL_A, INPUT_PULLUP);//oct select
  pinMode(OCT_SEL_B, INPUT_PULLUP);//oct select
  pinMode(PUSH_BUTTON, INPUT_PULLUP);//push sw
  pinMode(MODE_1, INPUT_PULLUP);//mode select
  pinMode(MODE_2, INPUT_PULLUP);//mode select
  timer = micros();
...
void loop()
{
  ...
  else if (digitalRead(MODE_1) == 1 && digitalRead(MODE_1) == 0) {
    mode = 2;//wavefolder
  }
